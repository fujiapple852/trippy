...
use windows_sys::Win32::Networking::WinSock::{
    ...
    TCP_ICMP_ERROR_INFO, WSA_INVALID_EVENT, WSA_IO_INCOMPLETE, WSA_IO_PENDING, WSABUF, WSADATA,
...
impl SocketImpl {
    ...
    fn create_event(&mut self) -> IoResult<()> {
        self.ol.hEvent = syscall!(WSACreateEvent(), |res| { res == WSA_INVALID_EVENT })
    ...
    fn reset_event(&self) -> IoResult<()> {
        syscall!(WSAResetEvent(self.ol.hEvent as WSAEVENT), |res| {
            res == 0
        })
